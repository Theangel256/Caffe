module.exports.run = async (client, message) => {
	...
	if (serverQueue.songs[0].voteStop.includes(message.member.id)) {
		...
		return message.channel.send(`Has quitado tu voto! ${serverQueue.songs[0].voteStop.length}/${required} requeridos.`);
	}
	...
	message.channel.send(`Voto para finalizar la cola ${serverQueue.songs[0].voteStop.length}/${required} requeridos.`);
};
