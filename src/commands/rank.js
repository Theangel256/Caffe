const Canvas = require('canvas'),
	{ getRank, getMember } = require('../structures/functions.js');
module.exports.run = async (client, message, args) => {
	...
	let niveles = await db.get(`${message.guild.id}.${message.author.id}`);
	if(!niveles) {
		db.set(`${message.guild.id}.${message.author.id}`, { xp: 0, lvl: 1 });
		niveles = await db.get(`${message.guild.id}.${message.author.id}`);
	}
	const { xp, lvl } = await db.get(`${message.guild.id}.${member.user.id}`);
	const usuarios = getRank(await db.get(message.guild.id), message);
