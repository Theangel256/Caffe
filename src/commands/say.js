module.exports.run = (client, message, args) => {
	...
	if (!args[0]) {return message.channel.send('Nada para decir? `Ej: /say Hola o /say embed Hola o /say image url.com`').then(m => m.delete({ timeout: 5000 }));}
	let emoji;
	...
	if(args[0].toLowerCase() === 'image') {
		if (!args[1]) {return message.channel.send('Nada para ver? `Ej: /say image url.com`').then(m => m.delete({ timeout: 5000 }));}
		if(!/https?:\/\/.+\.(?:png|jpg|jpeg|gif)/g.test(args[1])) return message.channel.send('esta no es una url, o no tiene el formato permitido `png, jpg, jpeg, gif` ejemplo: `/say image https://caffebot.glitch.me/api/example.png`');
	...
	else if (!emoji) {message.channel.send(args.join(' '));}
	else {
		const descReplace = new RegExp(`:${emoji.name}:`);
