...
module.exports.run = async (client, message, args) => {
	if (!message.channel.permissionsFor(message.member).has('MANAGE_MESSAGES')) {return message.reply(client.lang.userPerms.replace(/{function}/gi, missingPerms(client, message.member, ['MANAGE_MESSAGES'])));}
	if (!message.channel.permissionsFor(message.guild.me).has('MANAGE_MESSAGES')) {return message.reply(client.lang.clientPerms.replace(/{function}/gi, missingPerms(client, message.guild.me, ['MANAGE_MESSAGES'])));}

	if (!args[0] || (isNaN(args[0]) && !args[1])) return message.channel.send(client.lang.commands.clear.no_args);

	const number = args[1] ? parseInt(args[1]) : parseInt(args[0]);
