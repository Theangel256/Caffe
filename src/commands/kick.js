...
module.exports.run = (client, message, args) => {
	...
	if (member.user.id === message.author.id) return message.reply(client.lang.commands.kick.yourself);

	if (!member.kickable) return message.channel.send(client.lang.commands.kick.kickable);

	member.kick(reason);

	message.channel.send(client.lang.commands.kick.sucess.replace(/{user.tag}/gi, member.user.tag).replace(/{reason}/gi, reason));
};
module.exports.help = {
	name: 'kick',
