module.exports.run = async (client, message, args) => {
	...
	if(!args.join(' ')) return message.channel.send(`El volumen es: **${serverQueue.volume}**`);
	...
	if (countVolumen <= 5 && (countVolumen >= 0)) {
		...
		return message.channel.send(`**Volume:** \`${Math.round(serverQueue.volume * 50)}\`**%**`);

	}
	else {return message.channel.send('El volumen debe estar entre **0 a 5**');}
};
module.exports.help = {
	name: 'volume',
